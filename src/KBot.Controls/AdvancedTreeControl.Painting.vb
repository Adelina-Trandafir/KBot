...
Partial Public Class AdvancedTreeControl
    ...
    Private Sub DrawContent(g As Graphics, it As TreeItem, y As Integer, xBase As Integer)
        ...
        Dim nodeFontCreated As Boolean = False
        ...
        Try
            ...
            ' rowHasCols : ACEST rând primește celule?
            '              dinamic = toate rândurile; static = doar Level = ColumnsLevel
            ' Rândurile FĂRĂ coloane își întind captionul pe tot spațiul disponibil.
            Dim rowCols As List(Of ColumnDef) = Nothing
            If _treeListViewEnabled AndAlso _treeListView AndAlso RowHasColumns(it) Then
                rowCols = GetRowColumns(it)
            End If
            Dim rowHasCols As Boolean = (rowCols IsNot Nothing AndAlso rowCols.Count > 0)
            Dim captionAsColumn As Boolean = rowHasCols AndAlso String.IsNullOrEmpty(it.Caption)
            Dim colStartX As Integer = 0

            If rowHasCols Then
                colStartX = GetColumnStartX(rowCols)
                If ReferenceEquals(rowCols, _columns) Then
                    _captionColumnEndX = colStartX  ' citit de DrawColumnHeaders / GetColumnAtX
                End If
            End If

            ' Fundalul nodului acoperă și zona coloanelor (vizual identic cu înainte)
            Dim nodeBackWidth As Integer = Math.Max(0, maxRightX - textX)

            ' FIX: captionul se oprește la prima coloană pe rândurile cu coloane
            If rowHasCols AndAlso Not captionAsColumn Then maxRightX = Math.Min(maxRightX, colStartX)
            ...
            ' ══ 6. CAPTION (clip pe zona de text) ═════════════════════════════════
            ...
            ' ══ 7. CELULE TreeListView — DOAR pe randurile cu coloane ═════════════
            If rowHasCols Then
    ...
    Private Sub DrawColumnHeaders(g As Graphics)
        ...
        Try
            ...
            _captionColumnEndX = GetColumnStartX(_columns)
