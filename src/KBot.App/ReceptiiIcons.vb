...
''' <summary>
''' Iconițele stării receptiei (felia 0015), desenate din forme GDI, nu din resurse
''' binare: „sus" (receptie încărcată, triunghi în sus), „jos" (preluată, triunghi în
''' jos) și „neutru" (niciuna, bară). Oglindesc REV_SUS / REV_JOS / REV_NOT din Access.
''' Se desenează cu o culoare dată (din paleta temei), deci se re-tintează la schimbarea
''' temei. Cache pe (stare, culoare, dimensiune) — bitmap-urile sunt imutabile și
''' partajate, exact ca <see cref="FxIcons"/> / <see cref="RezervariIcons"/>.
''' </summary>
Public NotInheritable Class ReceptiiIcons

    ''' <summary>Starea vizuală a rădăcinii (receptie), derivată din Incarcat/Preluat.</summary>
    Public Enum Stare
        Neutru = 0
        Sus = 1
        Jos = 2
    ...
    ''' <summary>Iconița stării receptiei. Culoarea vine din paletă (verde/accent/estompat).</summary>
    Public Shared Function StatusIcon(stare As Stare, color As Color, size As Integer) As Image
        Return GetOrDraw($"rec:{stare}:{color.ToArgb()}:{size}", size,
    ...
    Private Shared Sub DrawStatus(g As Graphics, stare As Stare, color As Color, size As Integer)
        ...
        Using brush As New SolidBrush(color)
            Select Case stare
                Case Stare.Sus
                    ' triunghi cu vârful în sus (receptie încărcată)
                    g.FillPolygon(brush, New PointF() {
                        New PointF(size / 2.0F, m),
                        New PointF(size - m, size - m),
                        New PointF(m, size - m)})
                Case Stare.Jos
                    ' triunghi cu vârful în jos (receptie preluată)
                    g.FillPolygon(brush, New PointF() {
                        New PointF(m, m),
                        New PointF(size - m, m),
