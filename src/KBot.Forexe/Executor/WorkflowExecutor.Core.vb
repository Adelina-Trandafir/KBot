...
Partial Public Class WorkflowExecutor
    ...
    ' --- PIN MANUAL (A3) ---
    ' Când e True, partea de UI Automation (WindowsSecurityAutomation) doar auto-selectează
    ' certificatul, iar PIN-ul îl tastează utilizatorul în dialogul Windows (niciun SendKeys de PIN).
    ' Valoarea se propagă la monitorul de securitate dacă/când acesta este construit.
    ' Notă: în build-ul curent _windowsSecurityAutomation nu este instanțiat și
    ' StartMonitoring() nu este apelat nicăieri (vezi raport A3 §2.4), deci setter-ul este
    ' un seam fără efect activ; rămâne pentru cablarea viitoare a monitorului.
    Private _manualPinMode As Boolean = False
    Public Property ManualPinMode As Boolean
        Get
            Return _manualPinMode
        End Get
        Set(value As Boolean)
            _manualPinMode = value
            _windowsSecurityAutomation?.SetManualPinMode(value)
        End Set
    End Property
