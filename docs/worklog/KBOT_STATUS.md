...
- **Slice 0015 (Recepții) — pass 0015-01 landed, code green, 0 warnings; the third real
  ...
  on screen; (3) **pass 0015-02 (the receptie root tooltip) landed** — difhCum/platiCum/
  Diferență reconciliation as a `<table>` tooltip on each root (red/blue diff), computed
  from the `plati` + `difh` + `sters_h` already loaded (no new server call), 2 headless
  tests. The tooltip has never been seen on screen.
