...
**Slice 0006** (see `KBOT_STATUS.md`). Worklog:
`docs/worklog/SLICE-0006-mainform-scaffolding.md` (sub-number per pass). The POCO correction
is **Slice 0007** (`docs/worklog/SLICE-0007-angajament-tree-info.md`) — do it just before or
as part of this slice, since MainForm's tree binds to that POCO. Read `CODE_WORKFLOW.md`
before starting.
...
Not complete until: solution builds clean (zero warnings) AND a worklog exists at
`docs/worklog/SLICE-0006-mainform-scaffolding.md` (what changed and why · files touched ·
test results · anything left unverified or deferred) AND `KBOT_STATUS.md` is updated AND all
three are committed and pushed together. If any is missing, report the task as unfinished.
