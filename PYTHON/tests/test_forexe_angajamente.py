...
#   2) config.py is present on the host (API_KEY is imported from it — the same
#      value require_api_key compares against, so no key needs to be pasted here);
...
def test_missing_rows_returns_400(client):
    r = client.post(URL, headers=HEADERS, data=json.dumps({"db_name": DB_NAME}))
    assert r.status_code == 400


def test_rows_not_list_returns_400(client):
    r = client.post(URL, headers=HEADERS, data=json.dumps({"db_name": DB_NAME, "rows": "nope"}))
    assert r.status_code == 400


def test_empty_rows_is_success_zero_written(client):
    r = client.post(URL, headers=HEADERS, data=json.dumps({"db_name": DB_NAME, "rows": []}))
    assert r.status_code == 200
    assert r.get_json().get("written") == 0


def test_empty_cod_is_skipped(client):
    body = {"db_name": DB_NAME, "rows": [{"Cod": "  ", "Descriere": "x", "Stare": "y"}]}
    r = client.post(URL, headers=HEADERS, data=json.dumps(body))
    assert r.status_code == 200
    assert r.get_json().get("written") == 0


def test_get_missing_db_name_returns_400(client):
    r = client.get(GET_URL, headers={"X-Api-Key": API_KEY})
    assert r.status_code == 400


def test_get_missing_api_key_is_rejected(client):
    r = client.get(GET_URL + "?db_name=" + DB_NAME)
    assert r.status_code in (401, 403)


def test_get_returns_list_shape(client):
    r = client.get(GET_URL + "?db_name=" + DB_NAME, headers={"X-Api-Key": API_KEY})
    assert r.status_code == 200
    body = r.get_json()
...
def test_upsert_then_get_finds_the_row(client):
    ...
    try:
        ...
        assert client.post(URL, headers=HEADERS, data=json.dumps(body)).status_code == 200
...
def test_insert_then_update_refreshes_descriere_stare(client):
    ...
    try:
        ...
        assert client.post(URL, headers=HEADERS, data=json.dumps(body1)).status_code == 200
        body2 = {"db_name": DB_NAME, "rows": [{"Cod": "T1", "Descriere": "D2", "Stare": "S2"}]}
        assert client.post(URL, headers=HEADERS, data=json.dumps(body2)).status_code == 200
